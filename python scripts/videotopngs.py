...
OUTPUT_SLIDES_DIR = os.path.join(os.getcwd(), "output")
...
#            mask = cv2.dilate(mask, None, iterations=2)
        ...
        if p_diff < MIN_PERCENT and not captured and frame_count > WARMUP:
            captured = True
            filename = f"{str(int(frame_time)).zfill(4)}.png"
...
def initialize_output_folder(video_path):
    ...
    output_folder_screenshot_path = os.path.join(OUTPUT_SLIDES_DIR, os.path.basename(video_path))
